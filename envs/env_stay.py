...
class StandingEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        self.encoder_queue.append(np.array([self.filtered_roll, self.filtered_gy, drive_vel]))
        ...
        true_roll = angle[0]
        true_gyro = self.data.sensor("body_gyro").data[0]
        true_drive_vel = self.data.qvel[self.model.jnt_dofadr[self.l_wheel_id]]

        com_offset_x = self.model.body_ipos[1][0] - self.nominal_ipos[1][0]
        com_offset_y = self.model.body_ipos[1][1] - self.nominal_ipos[1][1]
        com_offset_z = self.model.body_ipos[1][2] - self.nominal_ipos[1][2]

        critic_obs = np.array([
            true_roll, true_gyro, true_drive_vel, 
            com_offset_x, com_offset_y, com_offset_z
        ], dtype=np.float32)

        # return {
        #     "actor_obs": np.concatenate(self.obs_history).astype(np.float32),
    ...
    def step(self, action):
        ...
        if self.env_cfg["real_syncro_noise"] == True:
            ...
            action_torque += np.random.normal(0, 0.0233 * 0.005)  # トルクにノイズを加える
